...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validateCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($formData['code'])) {
            ...
            $existing = fetchOne("SELECT id FROM coupons WHERE code = :code", ['code' => $formData['code']]);
            if ($existing) {
                $error = 'Este código de cupón ya existe';
            } else {
                $couponId = saveCoupon($formData);
                if ($couponId) {
                    $_SESSION['success_message'] = 'Cupón creado exitosamente';
                    header('Location: list.php');
                    exit;
                } else {
                    $error = 'Error al guardar el cupón';
                }
            }
        }
    }
    ...
    <form method="POST" style="max-width: 800px;">
        ...
        <div class="form-row">
            ...
            <div class="form-group" id="max-discount-group" style="display: none;">
                ...
                <small>Solo para descuentos por porcentaje</small>
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="datetime-local" id="valid_from" name="valid_from"
                       value="<?= htmlspecialchars($formData['valid_from'] ?? '') ?>">
                <small>Dejar vacío = disponible inmediatamente</small>
            </div>
            ...
            <div class="form-group">
                ...
                <small>Dejar vacío = sin fecha de expiración</small>
            </div>
        </div>
        ...
        <div class="form-group" id="product-group" style="display: none;">
            ...
            <small>ID del producto (slug)</small>
        </div>
        
        <div class="form-group">
            <label>
                <input type="checkbox" name="active" value="1" 
                       <?= isset($formData['active']) && $formData['active'] ? 'checked' : 'checked' ?>>
                Cupón activo
            </label>
        </div>

...
<body<?= (basename($_SERVER['PHP_SELF'] ?? '') == 'landing-page.php') ? ' class="landing-editor-page"' : '' ?>>
    ...
    <div class="admin-mobile-topbar" id="admin-mobile-topbar">
        ...
        <div class="admin-mobile-right">
                    ...
                    <?php if ($stockNotificationsCount > 0): ?>
                        <?php foreach (array_slice($outOfStockProducts, 0, 5) as $p): ?>
                            <a href="<?= ADMIN_URL ?>/edit.php?id=<?= htmlspecialchars($p['id']) ?>" class="admin-notification-item stock-out">
                                ...
                                <span class="notif-desc">Stock agotado</span>
                            </a>
                        <?php endforeach; ?>
                        <?php foreach (array_slice($lowStockProducts, 0, 5) as $p): ?>
                            <a href="<?= ADMIN_URL ?>/edit.php?id=<?= htmlspecialchars($p['id']) ?>" class="admin-notification-item stock-low">
                                ...
                                <span class="notif-desc">Quedan <?= $p['stock'] ?> (min: <?= $p['stock_minimo'] ?>)</span>
                            </a>
                        <?php endforeach; ?>
                    <?php else: ?>
            ...
            <button class="admin-mobile-toggle" id="admin-mobile-toggle" aria-label="Alternar menu">
    ...
    <div class="admin-layout">
        <aside class="admin-nav-sidebar" id="admin-nav-sidebar">
            <div class="admin-nav-brand">
                <div>
                    <span class="admin-nav-brand-text">Somos <span>Tiendi</span></span>
                    <small class="admin-nav-brand-shop"><?= htmlspecialchars($shopName) ?></small>
                </div>
            </div>
            ...
            <nav class="admin-nav-menu">
                ...
                <section>
                    <div class="menu-label">Accesos rapidos</div>
                ...
                <section>
                    <div class="menu-label">Catalogo</div>
                    <a href="<?= ADMIN_URL ?>/galeria/list.php" class="<?= (strpos($_SERVER['REQUEST_URI'] ?? '', 'galeria') !== false) ? 'active' : '' ?>">
                        <span class="nav-icon"><?= icon('image', 20) ?></span> Galeria
                    </a>
                    <a href="<?= ADMIN_URL ?>/categorias/list.php"><span class="nav-icon"><?= icon('folder', 20) ?></span> Categorias</a>
                    <a href="<?= ADMIN_URL ?>/cupones/list.php"><span class="nav-icon"><?= icon('ticket', 20) ?></span> Cupones</a>
                </section>
                <section>
                    <div class="menu-label">Configuracion</div>
                    <a href="<?= ADMIN_URL ?>/landing-page.php"><span class="nav-icon"><?= icon('home', 20) ?></span> Pagina de inicio</a>
                    <a href="<?= ADMIN_URL ?>/tienda.php"><span class="nav-icon"><?= icon('settings', 20) ?></span> Datos de la tienda</a>
                    <a href="<?= ADMIN_URL ?>/planes.php" class="<?= (basename($_SERVER['PHP_SELF']) == 'planes.php') ? 'active' : '' ?>"><span class="nav-icon"><?= icon('star', 20) ?></span> Plan y Suscripcion</a>
                ...
                <section>
                    <div class="menu-label">Informacion</div>
                    <a href="<?= ADMIN_URL ?>/reports/list.php"><span class="nav-icon"><?= icon('bar-chart', 20) ?></span> Reportes mensuales</a>
                </section>
            </nav>

            <div class="admin-nav-footer">
                <a href="<?= defined('CURRENT_STORE_SLUG') ? '/' . CURRENT_STORE_SLUG : '/' ?>" target="_blank"><?= icon('eye', 16) ?> Ver mi tienda</a>
            </div>
        </aside>
        ...
        <div class="admin-main-wrapper" id="admin-main-wrapper">
            <header class="admin-topbar">
                <div class="admin-topbar-right">
                    <a href="<?= ADMIN_URL ?>/planes.php" class="admin-plan-badge" title="Ver planes y suscripcion">
                    ...
                    <div class="admin-user-menu" id="admin-user-menu">
                        ...
                        <span style="font-size:0.6rem;color:#94a3b8;margin-left:0.15rem;">&#9660;</span>
                        <div class="admin-user-dropdown" id="admin-user-dropdown">
                            <a href="<?= ADMIN_URL ?>/perfil.php"><?= icon('user', 16) ?> Mi Perfil</a>
                            <a href="<?= ADMIN_URL ?>/logout.php"><?= icon('log-out', 16) ?> Cerrar sesion</a>
                        </div>
                    </div>
                </div>
            </header>
